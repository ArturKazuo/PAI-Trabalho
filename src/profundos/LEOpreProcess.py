...
def createSubImages(line, img):   #usado para criar subimagens e armazená-las
    ...
    cv2.imwrite('PAI-Trabalho\\classes\\' + classImg + "\\" + line[3] + ".png", roi_rgb) #armazena a subimgem na sua classe correta

def preProcess():

    directory = "PAI-Trabalho\classes"
    ...
    if not os.path.exists(directory):
        ...
        directory1 = "PAI-Trabalho\classes\\Negative"
        directory2 = "PAI-Trabalho\\classes\\ASCUS"
        directory3 = "PAI-Trabalho\\classes\ASCH"
        directory4 = "PAI-Trabalho\classes\LSIL"
        directory5 = "PAI-Trabalho\classes\HSIL"
        directory6 = "PAI-Trabalho\classes\SCC"
    ...
    delete_files_in_directory('PAI-Trabalho\classes\ASCH')
    delete_files_in_directory('PAI-Trabalho\classes\ASCUS')
    delete_files_in_directory('PAI-Trabalho\classes\LSIL')
    delete_files_in_directory('PAI-Trabalho\classes\HSIL')
    delete_files_in_directory('PAI-Trabalho\classes\\Negative')
    delete_files_in_directory('PAI-Trabalho\classes\SCC')
    
    #abre e lê a planilha

    file = open('PAI-Trabalho\database\classifications.csv')  
    ...
    for lines in content[1:len(content)]:           #itera sobre todas as linhas da planilha 
        ...
        if(line[1] != atual):                       #se o nome do arquivo for diferente do atual, significa que outra imagem deve ser lida
            ...
            check_file = os.path.isfile("PAI-Trabalho\database\dataset\\" + atual)   #checa para ver se o novo arquivo existe
            
            if(check_file):  
                print(atual)                                   #caso ele exista:
                img = cv2.imread("PAI-Trabalho\database\dataset\\" + atual)          #lê a nova imagem
